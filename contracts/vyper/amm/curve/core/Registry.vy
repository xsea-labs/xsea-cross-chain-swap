...
def _get_rates(_pool: address) -> uint256[MAX_COINS]:
    ...
    if base_pool == ZERO_ADDRESS:
        ...
        for i in range(MAX_COINS):
            _max_coin: uint256 = convert(i, uint256)
            if _max_coin == base_coin_idx:
                break
            rates[i] = 10 ** 18
...
def _get_meta_underlying_balances(_pool: address, _base_pool: address) -> uint256[MAX_COINS]:
    ...
    for i in range(MAX_COINS):
        ...
        if self.pool_data[_pool].ul_coins[i] == ZERO_ADDRESS:
            break
        if _i < base_coin_idx:
            ul_balance = CurvePool(_pool).balances(_i)
        else:
            if is_v1:
                ul_balance = CurvePoolV1(_base_pool).balances(convert(_i - base_coin_idx, int128))
            else:
                ul_balance = CurvePool(_base_pool).balances(_i - base_coin_idx)
...
def add_metapool(
    ...
    for i in range(MAX_COINS):
        _i: uint256 = convert(i, uint256)
        if _i == base_n_coins + base_coin_offset:
            break
        if _i < base_coin_offset:
            coin = coins[i]
        else:
    ...
    for i in range(MAX_COINS):
        if convert(i, uint256) == base_coin_offset:
            break
        for x in range(MAX_COINS):
            if convert(x, uint256) == base_n_coins:
                break
